...
router.route('/singup').post((req, res) => {
    User.findOne({
        ...
        if (!ok) {
            ...
            user.blocked = as;
            user.admin = "false",
                user.avate = "https://moki.vn/img/default-avatar.png?fbclid=IwAR02Ejs7IdaKgT75_ICYc2wSIuSPSVeMSQXMaW8MOakVtvtsc9HtUSl9ozw"
            user.save((err) => {
                if (err) res.json({ code: 1005, message: "Unknow err" })
                res.json({
                    code: 1000,
...
router.route('/changepass').post((req, res) => {
    let pass = bcrypt.hashSync(req.body.passnew);
    User.updateOne({
        phone: req.body.phone
    }, {
            pass: pass
        ...
        User.findOne({
            ...
            if (ok) {
                if (bcrypt.compareSync(req.body.pass, ok.pass)) {
                    console.log("ok");
                    let pass = bcrypt.hashSync(req.body.passnew);
                    console.log(pass + "    " + req.body.passnew);
                    User.updateOne({
                        phone: req.userData.phone
                    }, {
                            pass: pass
                    ...
                    res.json({
                        code: 1005,
                        message: "Unknow err or your pass is incorrect"
                    })
                }
            }
        })
            ...
            if (ok) {
                if (ok.auth == 1) {
                    ...
                    Product.find({}, (err, oki) => {
                        ...
                        product.id_owner = ok._id;
                        product.id = oki.length + 1;
                        product.img = req.body.img;
                        product.shop_name = ok.name;
                        product.numberproduct = req.body.soluong;
                        product.like = as;
                        product.imgavata = req.body.img[0];
                        product.dislike = as;
                        product.rate = as;
...
router.route('/get_comment_products').post((req, res) => {
    Product.findOne({
        ...
        if (ok) {
            ...
            let index = ok.comment.length - 1 - req.body.index;
            for (let i = ok.comment.length - 1; i >= 0; i--) {

                if (ok.comment[i].idcomment == index) {
                    for (let j = i; j >= 0; j--) {
                        ...
                        if (!m) {
                            User.updateOne({
                                phone: req.userData.phone
                                    ...
                                    phone: req.body.phone.length > 0 ? req.body.phone : ok.phone,
                                    email: req.body.email > 0 ? req.body.email : ok.email,
                                    name: req.body.name.length > 0 ? req.body.name : ok.name,
                ...
                if (p > 0) {
                    ...
                    User.updateOne({
                        phone: req.userData.phone
                            ...
                            User.findById(req.body.follower_id, (err, okew) => {
                                User.updateOne({
                                    _id: req.body.follower_id
                                }, {
                                        numfollowed: okew.numfollowed + 1
                                    }, (err, oki) => {
                                        if (err) throw err;
                                        res.json({
                                            code: 1000,
                                            message: "ok",
                                            data: okew.numfollowed + 1
                                        })
                                    })
                            })
...
router.route('/homeproduct').post((req, res) => {
    if (!req.userData) {
        res.json({
            code: 9998,
            message: "Token is not validated"
        ...
        User.findOne({
            ...
            if (ok1) {
                ...
                res.json({
                    code: 1005,
                    message: "Unkonw or you arent admin"
                })
            }
        })
    }
        ...
        if (ok.length > 0) {
            ...
            User.findById(
                ...
                , (err, oki) => {
                    ...
                    let re = [];
                    re.push(oki.time);
                    re.push(oki.numfollowed);
                    re.push(ok.length);
                    re.push(oki.avata);
                    re.push(oki.name);
                    // console.log(re+ok.avata)
                    res.json({
                        code: 1000,
                        message: "ok",
                        data: {
